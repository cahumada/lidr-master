...
El transcript durable de una conversación: los turnos cerrados que la ventana
de memoria nunca recorta, el snapshot de citas con que se puede verificar una
respuesta al reabrirla, y el listado de sesiones con su título.

No se confunde con `conversation-memory`: eso es lo que el prompt lee para
responder el turno siguiente; esto es lo que un operador vuelve a mirar.

Implementado en `app/generation/conversation/store.py` (el transcript),
`app/generation/conversation/models.py` (`HistoryTurn`, `CitationSnapshot`,
`SessionSummary`) y `app/api/answer_session.py`.

Promovido de: `add-conversation-history`.
