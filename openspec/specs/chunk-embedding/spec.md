...
## Purpose
Generar y persistir el vector de cada chunk del corpus, de forma reanudable e
incremental. Implementado en `app/generation/rag/embedding/`: `embedder.py`
(el protocolo `Embedder`, `OpenAIEmbedder` y el `HashEmbedder` determinístico
de tests), `sidecar.py` (el IO del `.npy` y su índice) y `runner.py`
(planificación, loop con checkpoints y verificación). El batch es
`scripts/embed_corpus.py`.

La persistencia en pgvector NO es parte de esta capability: embeber y persistir
son fallas distintas, y separarlas hace que el paso caro —la llamada al
modelo— no se vuelva a pagar cuando el barato falla.
