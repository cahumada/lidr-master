...
El consumo de tokens que los proveedores reportan: cómo llega al `Completion`,
cuándo se persiste una fila del ledger y cuándo no, qué viaja en la respuesta
HTTP, y el agregado por tenant.

Implementado en `app/foundation/llm/wrapper.py` (el `Usage` del `Completion` y
el `RecordingLLM`), `app/foundation/persistence/usage.py` (el ledger) y
`app/api/usage.py`.

Promovido de: `add-llm-usage-accounting`.
