...
Two broken shapes occur in the real corpus. Both SHALL be reconstructed into a
single valid markdown table with a header row and a `---` separator row, so the
chunker sees one table instead of orphaned headings and pipe-bearing lines.
