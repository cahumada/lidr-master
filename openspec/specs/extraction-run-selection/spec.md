# extraction-run-selection Specification

## Purpose

Con qué corrida del mirror de VisualTIME trabaja el servicio, quién la elige y
qué se rechaza. El mirror (`visualtime.*`) acumula corridas del extractor —lo
escribe otro repo y acá se lee— y el servicio lee de **una**: cuál es se elige
explícitamente, la elección vive en el esquema del servicio, y la configuración
es la semilla con la que arranca una instalación donde nadie eligió todavía.

Cubre además la contabilidad del desfasaje: con qué corrida se estampó la
metadata del corpus, para que la diferencia con la activa se muestre en lugar de
deducirse.

Implementado en `app/domain/business_db_store.py` (las dos tablas, la resolución
de la corrida vigente y la caída al valor por defecto), `app/api/business_db.py`
(el listado y la activación), `app/api/config.py` (la corrida vigente junto al
resto de la configuración) y `scripts/backfill_window_status.py` (el sello).

Promovido de: `add-extraction-run-selection`.
