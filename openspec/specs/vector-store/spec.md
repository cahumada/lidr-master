...
## Purpose
Persistir cada chunk del corpus con su vector y responder búsquedas por
similitud con filtros estructurales, aislando cliente y versión. Implementado en
`app/foundation/persistence/database.py` (engine y sesiones),
`app/generation/rag/store/` (`models.py` las tablas, `loader.py` la carga masiva
por `COPY`, `repository.py` las consultas) y `alembic/` (las migraciones). El
batch es `scripts/load_pgvector.py`.

El endpoint de búsqueda NO es parte de esta capability: expone decisiones que no
están tomadas —diversidad por documento, fusión con el full-text, reranking— y
son de `retrieval`.
...
<!-- Promovido de: fix-asyncpg-tls-mode -->
