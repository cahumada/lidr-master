...
- [ ] 1.1 `create-next-app` en `business-backend/` — TypeScript, App Router,
      Tailwind, ESLint.
- [ ] 1.2 Inicializar shadcn/ui. Copiar **solo** los componentes que las
      pantallas usan (`button`, `input`, `card`, `badge`, `table`, `switch`,
      `select`, `alert`) — no el catálogo entero.
- [ ] 1.3 `business-backend/.env.example` con `AI_SERVICE_URL` y nada más.
      Sin prefijo `NEXT_PUBLIC_`: si el browser la puede leer, el diseño se
      rompió.
- [ ] 1.4 Layout base con la navegación de las tres pantallas.

## 2. La capa que habla con el servicio IA
- [ ] 2.1 `lib/ai-service/base-client.ts`: `fetch` contra `AI_SERVICE_URL`,
...
- [ ] 2.2 `lib/ai-service/types.ts`: tipos que espejan 1:1 los schemas de
...
- [ ] 2.3 Un cliente por contexto: `search.ts`, `documents.ts`, `corpus.ts`.
      Los contextos no se importan entre sí.

## 3. Route Handlers (el proxy)
- [ ] 3.1 `GET /api/search` → reenvía los query params a `{AI_SERVICE_URL}/search`.
- [ ] 3.2 `POST /api/documents/ingest-file` → reenvía el multipart.
- [ ] 3.3 `POST /api/corpus/rebuild`, `GET /api/corpus/jobs`,
      ...
      reinterpretarlo.
- [ ] 3.4 Verificar que ninguna respuesta ni ningún bundle de cliente contiene
      `AI_SERVICE_URL` (grep sobre `.next/static` después de un build).

## 4. Pantallas
- [ ] 4.1 **Búsqueda**: formulario (`q`, `module_code`, `window_type_name`,
...
- [ ] 4.2 **Vista previa de ingesta**: subir un `.md` → chunks y estadísticas.
      Sin botón de guardar: esa operación no existe en el servicio.
- [ ] 4.3 **Reconstrucción de corpus**: selección de pasos
...
- [ ] 4.4 **Home**: enlaza las tres. Sin métricas en vivo — ningún endpoint las
      expone (ver `design.md`).

## 5. Despliegue del servicio IA (Railway)
- [ ] 5.1 `ai-service/Dockerfile`: `uv sync --frozen`, `uvicorn app.main:app`.
- [ ] 5.2 `ai-service/.dockerignore`: `data/`, `.venv/`, `evals/`, `tests/`.
...
- [ ] 5.5 Documentarlo en el `README.md` de la raíz.
...
- [ ] 6.2 "Ignored Build Step" que salte el build cuando el commit no toca
      `business-backend/`.
- [ ] 6.3 Documentarlo en el `README.md` de la raíz.

## 7. CI
- [ ] 7.1 `.github/workflows/ci.yml`: job `changes` con `dorny/paths-filter`
      (`ai-service`, `business-backend`).
- [ ] 7.2 Job del servicio IA: `uv sync --frozen`, `ruff check .`, `pytest -q`,
      `working-directory: ai-service`, condicionado al filtro.
- [ ] 7.3 Job de la app web: `npm ci`, `npm run lint`, `npm run build`,
      `working-directory: business-backend`, condicionado al filtro.
- [ ] 7.4 Ningún job de deploy (§5, §6).

## 8. Documentación
- [ ] 8.1 `openspec/project.md`: la stack de `business-backend/` (Next.js,
...
- [ ] 8.2 `README.md` de la raíz: cómo correr las dos apps en local y cómo se
      despliega cada una.
- [ ] 8.3 `AGENTS.md` §4: los comandos de `business-backend/`.

## 9. Verificación
- [ ] 9.1 `npm run lint` y `npm run build` en verde.
- [ ] 9.2 Desde `ai-service/`: `uv run pytest` y `uv run ruff check .` siguen
      en verde (este change no debería tocar nada del servicio; si algo falla,
      es que sí lo tocó).
- [ ] 9.3 `python scripts/validate_specs.py` en verde.
- [ ] 9.4 Smoke local: las tres pantallas contra
