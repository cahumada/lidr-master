...
- [ ] 1.1 `SERVICE_TOKEN: str = ""` en `app/config.py`, con el comentario
  bilingüe de por qué vacío significa abierto y en qué caso no se tolera.
- [ ] 1.2 Validación de `Settings`: con `APP_ENV=production` y `SERVICE_TOKEN`
...
- [ ] 1.3 `.env.example` de `ai-service/` con la variable y una línea de qué
  pasa si falta.

## 2. La dependencia que exige el header
- [ ] 2.1 En `app/dependencies.py`, una dependencia que lea
...
- [ ] 2.2 Sin token configurado, la dependencia **pasa** (el entorno está
  ...
  **401** con `WWW-Authenticate: Bearer` y sin decir cuál de las dos cosas
  falló.
- [ ] 2.3 Aplicarla en `app/main.py` a nivel de router, no endpoint por
  endpoint: un endpoint nuevo tiene que quedar cerrado por default, y eso solo
  pasa si el default es del router.
- [ ] 2.4 `/health` queda **afuera**. Es el healthcheck del despliegue: cerrarlo
  hace que Railway reinicie el servicio en loop.

## 3. El BFF adjunta el token
- [ ] 3.1 En `business-backend/lib/ai-service/base-client.ts`, el header en
  `call()` — el único lugar donde la consola hace `fetch` al servicio.
- [ ] 3.2 La variable **sin** prefijo `NEXT_PUBLIC_`: ese prefijo es el
  mecanismo que la expondría al browser.
- [ ] 3.3 Si la variable no está configurada, no mandar el header (el servicio
  puede estar abierto a propósito en desarrollo). **No** inventar un valor.
- [ ] 3.4 `.env.example` de `business-backend/` con la variable.

## 4. Tests
- [ ] 4.1 `tests/api/`: con token configurado, sin header → **401**; con el
  header correcto → la respuesta normal; con un header incorrecto → 401.
- [ ] 4.2 `/health` responde 200 sin header, con token configurado.
- [ ] 4.3 Sin `SERVICE_TOKEN`, todo responde como hoy — la regresión que importa,
  porque es el modo en que corren los tests y los evals.
- [ ] 4.4 `APP_ENV=production` sin `SERVICE_TOKEN` hace fallar la construcción de
  `Settings`, y el error nombra la variable.
- [ ] 4.5 Un endpoint agregado a un router protegido queda protegido sin tocar
  nada más — el test que fija que el default sea cerrado.
- [ ] 4.6 `uv run pytest` y `uv run ruff check .` en verde desde `ai-service/`.
  **Una sola suite a la vez**: los tests con base afirman conteos exactos contra
  el Postgres compartido.
- [ ] 4.7 `pnpm lint` y `pnpm build` desde `business-backend/`.

## 5. Specs y estándares
- [ ] 5.1 Delta de la capability nueva en
  `specs/service-authentication/spec.md`.
- [ ] 5.2 Delta en `specs/web-console/spec.md`: la capa que habla HTTP adjunta el
  token.
...
- [ ] 5.4 `python scripts/validate_specs.py` sin errores desde la raíz.
